...
class UsersRepository {
    ...
    async createUser(data) {
        return database_1.prisma.$transaction(async (tx) => {
            ...
            if (existing?.deletedAt) {
                throw new Error("User with this email exists but is soft-deleted. Restore account instead of recreating.");
            }
            if (existing) {
    ...
    async count() {
        return database_1.prisma.user.count({ where: { deletedAt: null } });
    }
    ...
    async countSuperAdmins() {
        return database_1.prisma.user.count({ where: { role: 'SUPER_ADMIN', deletedAt: null } });
    }
